# LEGACY: This script preserves the old WinPE-centric ISO assembly path.
...
try {
    ...
    $targetAppScriptsRoot = Join-Path $targetAppRoot "scripts"
    $targetLiveOsRoot = Join-Path $mountPath "CigerToolLive"
    $targetLiveOsShellRoot = Join-Path $targetLiveOsRoot "shell"
    $targetLiveOsStartupRoot = Join-Path $targetLiveOsRoot "startup"
    New-Item -ItemType Directory -Force -Path $targetAppRoot | Out-Null
    New-Item -ItemType Directory -Force -Path $targetAppScriptsRoot | Out-Null
    New-Item -ItemType Directory -Force -Path $targetLiveOsShellRoot | Out-Null
    New-Item -ItemType Directory -Force -Path $targetLiveOsStartupRoot | Out-Null
    Copy-Item -Path (Join-Path $appRoot "*") -Destination $targetAppRoot -Recurse -Force
    foreach ($operationScript in @("invoke_smart_clone.ps1", "invoke_raw_clone.ps1", "invoke_boot_fix.ps1")) {
        $sourceScript = Join-Path $projectRoot ("build\scripts\" + $operationScript)
        Assert-Path -PathValue $sourceScript -Description ("operasyon scripti " + $operationScript)
        Copy-Item -LiteralPath $sourceScript -Destination (Join-Path $targetAppScriptsRoot $operationScript) -Force
    }
    Copy-Item -Path (Join-Path $projectRoot "liveos\shell\*") -Destination $targetLiveOsShellRoot -Recurse -Force
    Copy-Item -Path (Join-Path $projectRoot "liveos\startup\*") -Destination $targetLiveOsStartupRoot -Recurse -Force
    Copy-Item -Path (Join-Path $projectRoot "winpe\files\*") -Destination $mountPath -Recurse -Force
    $commitChanges = $true
    Write-BuildLog "Live environment startup zinciri, operasyon scriptleri ve uygulama image icine kopyalandi."
}
finally {
    if ($mounted) {
        if ($commitChanges) {
